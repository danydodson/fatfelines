  ...
  return (
    ...
    >
      {width > breakpoint ? (
        ...
        <>
          <div onClick={toggleModal} className='absolute w-full h-full z-10 flex flex-col justify-between p-6'>
            <div className='flex justify-end gap-4'>
              <Icon name='Heart' />
              <Icon name='Plus' />
            </div>
            ...
            <article className="rounded overflow-hidden shadow-lg">
              ...
              <div className="px-6 pt-4 pb-2">
                {post.attributes.map((attr, index) => (
                  <span key={index} className="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 mr-1 rounded dark:bg-blue-900 dark:text-blue-300">
                    {attr}
                  </span>
                ))}
              </div>
